...
class Route
{
	...
	protected static $pre_route_objects = [];

	protected static $route_objects = [];

	protected static $pattern_list = [];
	 ...
	 */
	public static function map($routes = [],$languages = [], $module_version = null)
	{
		...
		foreach (self::$route_objects as $key => $route_object) {
			...
			if ($add_route) {
				...
				$to = ($module_version ? $module_version ."/" : "") . $to;
				if (!$languages) {
					$routes[$from] = str_replace('{default_controller}', $controller, $to);
				}else{
					foreach ($languages as $key => $lang) {
						$lang = $lang ? $lang . '/' : '';
						$routes[$lang.$from] = str_replace('{default_controller}', $controller, $to);
					}
				}
			}
		}
	...
	public static function match(array $requests, $from, $to, $options = [], $nested = FALSE)
	{
		$return = NULL;
	 ...
	 */
	public static function resources($name, $options = [], $nested = FALSE)
	{
		if (empty($name)) {
			return;
		}
	 ...
	 */
	public static function named($name, $parameters = [])
	{
	 ...
	 * @return void
	 */
	public static function context($name, $controller = NULL, $options = [])
	{
	 ...
	 */
	static function createRoute($from, $to, $options = [], $nested = FALSE)
	{
